...
Renderer::Renderer( void )
{
  ...
  m_Renderer = SDL_CreateRenderer( MainWindow_Ref.GetSDLWindowPtr(), FIRST_ONE, SDL_RENDERER_ACCELERATED );

  if( m_Renderer == NULL )
  {
    printf( "\tRenderer could not be created! SDL Error: \"%s\"\n", SDL_GetError() );
    printf( "\tMainWindow::Get() = %p\n", static_cast<void*>( MainWindow_Ref.GetSDLWindowPtr() ) );
...
{
  const int WINDOW_W = MainWindow_Ref.GetWindowWidth();
  const int WINDOW_H = MainWindow_Ref.GetWindowHeight();
